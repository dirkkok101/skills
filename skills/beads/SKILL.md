...
One bead = one pattern artifact. Split when a bead touches multiple pattern docs, spans projects, or combines data flow directions. Context budgets: BRIEF 5 files, STANDARD 8, COMPREHENSIVE 12. Frontend beads should be coarser than backend (coherent UI slices, not one component per bead). Trivially small beads may combine per grouping exceptions; entity beads must include migrations.

See [references/decomposition-tables.md](references/decomposition-tables.md) for the full sizing rules, split signals, context budgets, grouping exceptions, never-combine rules, compilation unit check, frontend coarseness rule, and verification batching.

---

## Bead Decomposition Tables

Backend: 19-bead .NET/Angular exemplar (entity → EF config → contracts → mappers → commands/queries → endpoints → validators → registration). Frontend: 7-bead exemplar (models → service → list/capture pages → state → routing). Not every entity needs all beads — check design docs to determine which apply. Test beads target ≤15 tests each; split into per-area beads for larger test plans.

For non-.NET projects, use the Decomposition Adaptation Algorithm to build tables from your `docs/patterns/` directory.

See [references/decomposition-tables.md](references/decomposition-tables.md) for the full backend, frontend, and test decomposition tables, plus the adaptation algorithm for other tech stacks.

---

## Test Gates (NOT Review/Simplify Gates)

Do NOT insert `/review` or `/simplify` gate beads between implementation beads. Instead, insert **test gates** at semantic boundaries: backend integration tests block all frontend beads; UI tests block UC gates. Frontend beads MUST depend on the backend test gate, never on raw backend impl beads.

Gate types: test gates (per feature, backend + frontend), UC verification gates (per use case), module completion gate (last bead in epic). Trust hierarchy: ADRs > PRD > Design docs > Plans > Beads > Implementation.

See [references/decomposition-tables.md](references/decomposition-tables.md) for gate placement rules, UC verification gates, module completion gates, dependency flow visualization, and trust hierarchy.
